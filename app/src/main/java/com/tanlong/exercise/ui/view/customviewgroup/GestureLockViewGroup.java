...
import com.tanlong.exercise.ui.view.customview.GestureLockView;
import com.tanlong.exercise.ui.view.customview.GestureLockView.Mode;
...
public class GestureLockViewGroup extends RelativeLayout {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mGestureLockViews == null) {
            ...
            for (int i = 0; i < mGestureLockViews.length; i++) {
                ...
                //设置参数，主要是定位每一个GestureLockView的位置
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (child != null) {// 有符合条件的GestureLockView
                    int cId = child.getId();
                    if (!mChoose.contains(cId)) {// 还没有保存这个GestureLockView
                        mChoose.add(cId);
                        ...
                        if (mChoose.size() == 1)// 当前添加的是第一个GestureLockView
                        {
    ...
    private boolean checkPositionInChild(View child, int x, int y) {
        ...
        //这里设置了内边距，即x,y必须落入GestureLockView的内部中间的小区域中，可以通过调整padding使得x,y落入范围不变大，
        // 或者不设置padding
    ...
    /**
     * 通过触摸事件的x,y坐标获得落入的GestureLockView
     *
     * @param x -- 触摸点的X坐标
     * @param y -- 触摸点的Y坐标
     * @return 如果有，返回对应的GestureLockView，否则返回null
     */
    ...
    /**
     * 分发给子组件进行绘制；ViewGroup容器组件的绘制，当它没有背景时直接调用的是dispatchDraw()方法,
    ...
    public interface OnGestureLockViewListener {
        /**
         * 选中GestureLockView,第一次选中时有效
         *
         * @param cId -- 被选中GestureLockView的ID
         */
